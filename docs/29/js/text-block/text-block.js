(function () { // window.TextBlock, get(sync/async string/URL):Generator, gets(sync/async string/URL):Array<string>
    class Lexer {
        ...
        /*
        async *get(source, withIndex=false) {
        ...
        *getFromString(text, withIndex=false) {
            text = this.#init(text)
            console.debug(text, this.#lines, this.#lines[0].length)
            if (1===this.#lines.length && 0===this.#lines[0].length) { return null }
        ...
        */
//        async *get(source, withIndex=false) { return this.#iter((await this.#getTextAsync(source)), withIndex) }
//        *getFromString(text, withIndex=false) { return this.#iter(text, withIndex) }
        async *get(source, withIndex=false) { yield* this.#iter((await this.#getTextAsync(source)), withIndex) }
        *getFromString(text, withIndex=false) { yield* this.#iter(text, withIndex) }
        *#iter(text, withIndex=false) {
            ...
            for (var i=0; i<this.#lines.length; i++) {
                ...
                if (isPush) {
                    const block = this.#parse(this.#lines.slice(start, end+1).join('\n'))
                    //yield await ((withIndex) ? [block, this.#idx] : block)
                    yield ((withIndex) ? [block, this.#idx] : block)
                    //yield (withIndex) ? [block, this.#idx] : block
                    this.#idx += 1
                }
...
//            console.log('#getText:')
        ...
        async gets(source) {
            ...
            const text = await this.#getTextAsync(source)
            return this.getsFromString(text)
        }
        /*
        getsAsync(source) {
//            console.log(source)
