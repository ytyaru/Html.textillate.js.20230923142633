window.addEventListener('DOMContentLoaded', async(event) => { // https://github.com/jschr/textillate
    console.log('DOMContentLoaded!!');
    gsap.registerPlugin(TextPlugin);
    //let message = getTween()
    let message = null
    const blocks = await TextBlock.fromUrl('js/text-block/test.txt')
    let idx = 0
    document.querySelector('#go').addEventListener('click', async(e) => {
        if (blocks.length <= idx) { return }
        document.querySelector('#go').disabled = true
        document.querySelector('#text').value = blocks[0]
        //message?.reverse()
        //message?.reset()
        gsap.from(`#message`,{text:'',ease:`none`})
        //message = gsap.to(`#message`, {
        gsap.to(`#message`, {
            ...
            ease: `none`,
            //paused: true,
            //onComplete: (p1,p2)=>{document.querySelector('#go').disabled = false; idx+=1;},
            onComplete: (p1,p2)=>{document.querySelector('#go').disabled=false; idx+=1;},
            //onStart: ()=>{ gsap.set(`#message`,{text:''}) }
            //onStart: ()=>{ gsap.from(`#message`,{text:''}) }
            //onStart: ()=>{message.reverse();}
            //onStart: ()=>{message.invalidate();}
            //onStart: ()=>{document.querySelector('#message').innerHTML='';console.log(document.querySelector('#message').innerHTML);}
        });
        /*
        for await (let [block, i] of TextBlock.fromUrlIter('js/text-block/test.txt', true)) {
            console.log(block)
            document.querySelector('#go').value = block
            gsap.to(`#message`, {
                duration: document.querySelector('#duration').value,
                text: block,
                ease: `none`,
                paused: true,
                onComplete: (p1,p2)=>{document.querySelector('#go').disabled = false}
            });
        }
        */
    })
